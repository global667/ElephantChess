...
void BoardView::PrepareEuroPiece(QPicture *pix, int row, int col, int h,
                                 int w) {
    ...
    const QString path = ":/res/" + piece->euroName + "_" +
                         (piece->getColor() == Color::Red ? "r" : "b") + "_z.png";
    ...
    QPointF center(w / (2.0 * PIECE_SCALE_FACTOR * cutpWidth),
                   h / (2.0 * PIECE_SCALE_FACTOR * cutpHeight));
    qreal radius =
        qMin(w, h) / (2.0 * PIECE_SCALE_FACTOR * qMax(cutpWidth, cutpHeight));
    ...
    const auto wdth = static_cast<double>(width) /
                      static_cast<double>(cutpWidth) / PIECE_SCALE_FACTOR;
    ...
    for (int j = 0; j < 10; j++) {
        for (int i = 0; i < 9; i++) {
            if (!basemodel.position.board[j][i]->getName().isEmpty()) {
                ...
                auto x = (50. + ((i)*marginLeft / cutpWidth)) -
                         width / cutpWidth / 2 / PIECE_SCALE_FACTOR;
                auto y = (50. + (9 - j) * marginRight / cutpHeight) -
...
void BoardView::PrepareNativePiece(QPicture *pix, const int row, const int col,
    ...
    // Draw piece name
    // QRectF textRect(center - QPointF(radius, radius / 2), QSizeF(2 * radius,
    // radius)); QRectF textRect(x, y, wght, hght);
...
void BoardView::PaintBoard(QPainter *p) const {
    ...
    // Palaeste
    p->drawLine(50 + (3 * squareWidth), 50 + 0 * squareHeight,
                50 + (5 * squareWidth), 50 + 2 * squareHeight);

    p->drawLine(50 + (3 * squareWidth), 50 + 2 * squareHeight,
                50 + (5 * squareWidth), 50 + 0 * squareHeight);

    p->drawLine(50 + (3 * squareWidth), 50 + 9 * squareHeight,
                50 + (5 * squareWidth), 50 + 7 * squareHeight);

    p->drawLine(50 + (3 * squareWidth), 50 + 7 * squareHeight,
                50 + (5 * squareWidth), 50 + 9 * squareHeight);

    // Vertikale Linien
    for (int i = 0; i <= cutpWidth; i++) {
        p->drawLine(50 + (i * squareWidth), 50, 50 + (i * squareWidth),
                    height - 100);
    }

    // Horizontale Linien
    for (int i = 0; i <= cutpHeight; i++) {
        p->drawLine(50, 50 + i * squareHeight, width - 50, 50 + i * squareHeight);
    }
    ...
    if (true) {
        ...
        // Horizontaler Text
        for (int i = 0; i <= cutpWidth; i++) {
            p->drawText(50 + (i * squareWidth), height - (2 * 50 / 2),
    ...
    if (event->type() == QEvent::ToolTip) {
        ...
        for (int j = 0; j < 10; j++) {
            for (int i = 0; i < 9; i++) {
                ...
                if (helpEvent->pos().x() > x &&
                    helpEvent->pos().x() < x + width / cutpWidth / PIECE_SCALE_FACTOR &&
                    helpEvent->pos().y() > y &&
                    helpEvent->pos().y() <
                                                                                                                                                     y + height / cutpHeight / PIECE_SCALE_FACTOR &&
                    !basemodel.position.board[j][i]->getName().isEmpty()) {
                    QToolTip::showText(
                        helpEvent->globalPos(),
                        QString("%1:\n%2")
                            .arg(basemodel.position.board[j][i]->getEuroName())
                            .arg(basemodel.position.board[j][i]->getEuroNameDesc()),
                        this, rect());
                    return true;
                }
            }
        ...
        //    %2").arg(helpEvent->pos().x()).arg(helpEvent->pos().y()), this,
        //    rect());
        // }
        return true;
    } else {
...
void BoardView::PaintSelectedPieces(QPainter *painter) const {
    ...
    if (pressed == true && secondclick == false) {
        ...
        const auto x =
            (50 + (((basemodel.fromHuman.y)) * (width - 2 * 50) / cutpWidth)) -
                       width / cutpWidth / 2 / PIECE_SCALE_FACTOR;
        const auto y = (50 + (9 - (basemodel.fromHuman.x)) * (height - 50 - 100) /
                                 cutpHeight) -
                       height / cutpWidth / 2 / PIECE_SCALE_FACTOR;
        ...
        auto all_moves = basemodel.position.board[fromX][fromY]->generateValidMoves(
            Point(fromX, fromY), basemodel.position.board);
...
void BoardView::DrawEngineMoves(QPainter *painter) const {
    ...
    painter->setPen(pen);
    painter->drawEllipse(
        QRect(x1 - width / cutpWidth / 2 / 1.5, y1 - height / cutpWidth / 2 / 1.5,
              width / (cutpWidth) / 1.5, height / cutpHeight / 1.5));

    painter->drawEllipse(
        QRect(x2 - width / cutpWidth / 2 / 1.5, y2 - height / cutpWidth / 2 / 1.5,
              width / (cutpWidth) / 1.5, height / cutpHeight / 1.5));
...
void BoardView::mousePressEvent(QMouseEvent *event) {
    ...
    const Point coord{10 - boardPos.y,
                      boardPos.x - 1}; // Simplified coordinate conversion
    if (!pressed) {
        pressed = true;
        if (const Piece piece = *basemodel.position.board[coord.x][coord.y];
            piece.getName().isEmpty() ||
            piece.getColor() != basemodel.position.players_color) {
        ...
        if (basemodel.position.board[coord.x][coord.y]->getColor() ==
        ...
        pressed = false;
        auto all_moves = Board::getAllValidMoves(basemodel.position.players_color,
                                                 basemodel.position.board);
        for (const auto &[from, to] : all_moves) {
            if (from == basemodel.fromHuman && to == coord) {
                // emit updateView(basemodel.fromHuman, basemodel.toHuman,
                // basemodel.mode);
...
constexpr float BOARD_COL_POINTS =
    BaseModel::BoardColPoints; // Assuming this is a valid static constant
constexpr float BOARD_ROW_POINTS =
    BaseModel::BoardRowPoints; // Assuming this is a valid static constant
...
Point BoardView::CalcBoardCoords(Point r) const {
    ...
    const float squareColWidth =
        (w - LEFT_MARGIN - RIGHT_MARGIN) / BOARD_COL_POINTS;
    const float squareRowHeight =
        (h - TOP_MARGIN - BOTTOM_MARGIN) / BOARD_ROW_POINTS;
      ...
      for (auto &marker: contextMenu->markers) {
          ...
          switch (marker.second) {
              ...
              case markerType::Kreis:
                  ...
                  p->drawChord(QRect((50 + (((col - 1)) * (w - 2 * 50) /
     cutpWidth)) - w / cutpWidth / 2 / 1.5, (50 + ((row - 1)) * (h - 50 - 100) /
     cutpHeight) - h / cutpWidth / 2 / 1.5, w / (cutpWidth) / 1.5, h / cutpWidth
     / 1.5), 0, 5760);
              ...
              case markerType::Linie:
                  ...
                  p->drawPoint((50 + (((col - 1)) * (w - 2 * 50) /
                                      cutpWidth)), // - w / cutp_width / 2
     / 1.5, (50 + ((row - 1)) * (h - 50 - 100) / cutpHeight));
              //- h / cutp_width / 2 / 1.5);
                  lrow = row;
                  lcol = col;
                  break;
              ...
              case markerType::Viereck:
                  ...
                  p->drawRect(QRect((50 + (((col - 1)) * (w - 2 * 50) /
     cutpWidth)) - w / cutpWidth / 2 / 1.5, (50 + ((row - 1)) * (h - 50 - 100) /
     cutpHeight) - h / cutpWidth / 2 / 1.5, w / (cutpWidth) / 1.5, h / cutpWidth
     / 1.5)); break; default: break;
          }
      }
  */
}
...
/ 1.5), 0, 5760);
        ...
        p->drawText(QRect((50 + (((i_col)) * (w - 2 * 50) / cutpWidth)) -
                                                  w / cutpWidth / 2 / 1.8,
                                          (50 + (i_row) * (h - 50 - 100) /
cutpHeight) - h / cutpWidth / 2 / 2, w / (cutpWidth), h / cutpWidth),
...
{ pixm = QPixmap::fromImage(basemodel.position.board[j][i].piece->img); pixm2 =
pixm.copy(100, 0, 100, 100); } else if (basemodel.viewStyleModeVar ==
                                         viewStyleMode::traditional_png) {
                        pixm =
QPixmap::fromImage(basemodel.position.board[j][i].piece->img, Qt::PreferDither);
                  ...
                  if (basemodel.viewStyleModeVar == viewStyleMode::western_png)
{ pixm = QPixmap::fromImage(basemodel.position.board[j][i].piece->img); pixm2 =
pixm.copy(300, 0, 100, 100); } else if (basemodel.viewStyleModeVar ==
                                         viewStyleMode::traditional_png) {
                        pixm =
QPixmap::fromImage(basemodel.position.board[j][i].piece->img, Qt::PreferDither);
