...
public:
    ...
    static std::vector<std::pair<Point, Point>> getAllValidMoves(
        Color color,
        const std::vector<std::vector<std::shared_ptr<Piece>>> &board);

    // std::vector<std::pair<Point, Point>> getValidMovesForPiece(const Point&
    // position, const //std::vector<std::vector<std::shared_ptr<Piece>>>& board);

    static bool
    movePiece(const Point &from, const Point &to,
              std::vector<std::vector<std::shared_ptr<Piece>>> &board);

    // undo a move on specific board
    static void undoMove(const Point &from, const Point &to,
