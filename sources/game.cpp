    ...
    Game::Game(QTreeWidget *table, MainWindow *parent) : table(table), parent(parent)  {}
    ...
    if (basemodel.position.players_color == Color::Red && basemodel.fromHuman.x != -1 && basemodel.fromHuman.y != -1 && basemodel.toHuman.x != -1 && basemodel.toHuman.y != -1) {
        mutex.lock();
...
if (basemodel.position.players_color == Color::Black && basemodel.fromUCI.x != -1 && basemodel.fromUCI.y != -1 && basemodel.toUCI.x != -1 && basemodel.toUCI.y != -1) {
    mutex.lock();
    ...
    void Game::AddMoveToList(const std::pair<Point, Point> move) const {
